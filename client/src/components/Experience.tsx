          ...
          <div className="space-y-16">
            {experiences.map((experience, index) => (
              <div key={index} className={`relative flex items-center transition-all duration-700 delay-${index * 200} ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
                <div className={`w-full md:w-2/5 ${index % 2 === 0 ? 'md:text-right' : 'md:order-2'}`} style={{marginRight: index % 2 === 0 ? '150px' : '0', marginLeft: index % 2 !== 0 ? '150px' : '0', width: 'calc(40% + 50px)'}}>
                ...
                <div className={`w-full md:w-2/5 ${index % 2 === 0 ? '' : 'md:order-1'}`} style={{marginLeft: index % 2 === 0 ? '150px' : '0', marginRight: index % 2 !== 0 ? '150px' : '0', width: 'calc(40% + 50px)'}}></div>
              </div>
            ))}
          </div>
        </div>
